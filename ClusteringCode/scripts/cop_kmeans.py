...
def cop_kmeans(dataset, k, ml=[], cl=[],
        ...
        for i, d in indexed:
            if clusters_[i] == -1:
                ...
                if not found_cluster:
                    print("Cluster not found for datapoint "+str(i))
                    return None, centers, it, shifts
    ...
    return clusters_, centers_, it, shifts
...
def initialize_centers(dataset, k, method):
    #Random: aleatoriamente escolhe k dados para serem centroides
    if method == 'random':
        ids = list(range(len(dataset)))
        random.shuffle(ids)
        return [dataset[i] for i in ids[:k]]
    
    #Kmeans++: escolhe centroides de forma que dados mais distantes dos centroides ja existentes 
    #          tenham maior probabilidade de serem escolhidos como proximo centroide
    elif method == 'kmpp':
...
# Monta os grupos de dados que devem ficar juntos segundo as regras must-link
